...
def augment_d4(s1: torch.Tensor) -> list[torch.Tensor]:
    symetries = []
    ...
    for k in range(4):
        ...
        symetries.append(ns_rot.flatten().to(device))
        ns_m = mirror_grid(ns_rot)
        symetries.append(ns_m.flatten().to(device))
...
class Value(nn.Module):
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        values = []
        for s in augment_d4(state):
            values.append(self.value(s).squeeze(-1))
        # pooling de symétrie
