...
_INVPERM_CACHE: dict[torch.device, torch.Tensor] = {}

def _perm_on_device(device: torch.device) -> torch.Tensor:
    # (8,81) int64 on device (cached)
    t = _PERM_CACHE.get(device)
    if t is None:
        t = torch.from_numpy(_ACTION_PERM_NP).to(device=device, dtype=torch.long)
        _PERM_CACHE[device] = t
    return t

def _invperm_on_device(device: torch.device) -> torch.Tensor:
    # invperm[k, a_new] = a_old (cached)
    t = _INVPERM_CACHE.get(device)
    if t is None:
        perm = _perm_on_device(device)
        inv = torch.empty_like(perm)
        # perm[k, a_old] = a_new => inv[k, a_new] = a_old
        a_old = torch.arange(perm.shape[1], device=device)
        for k in range(perm.shape[0]):
            inv[k, perm[k]] = a_old
        _INVPERM_CACHE[device] = inv
        t = inv
    return t
...
def _augment_symmetries(
    ...
    NOTE: logp_old is carried through but becomes invalid because (obs, action, mask) change.
    The caller MUST recompute logp_old after augmentation (pre-update).
    ...
    """
    device = obs.device
    perm = _perm_on_device(device)       # (8,81) a_old -> a_new
    invperm = _invperm_on_device(device) # (8,81) a_new -> a_old
    N = obs.shape[0]
    ...
    # We keep scalars replicated (no copy)
    logp_out = logp_old.repeat(8)
    returns_out = returns.repeat(8)
    adv_out = adv.repeat(8)

    for k in range(8):
        o_k = _transform_board_batch(obs, k)
        # masks_new[a_new] = masks_old[a_old] where a_old = invperm[a_new]
        m_k = masks[:, invperm[k]]
        a_k = perm[k][actions]
        obs_out.append(o_k)
...
# NOTE: GPU-optimized path uses a tensor RolloutBuffer (no Python objects per step).
# The Transition dataclass is kept only for compatibility with older code paths.
@dataclass
class Transition:
    env_id: int
    obs: torch.Tensor
...
class RolloutBuffer:
    """
    Fixed-size rollout buffer: shapes (T, n_envs, ...).

    Stored on CPU pinned memory by default, then transferred once to GPU for GAE+PPO.
    Obs is stored as uint8 (0/1) to reduce bandwidth.
    """
    def __init__(self, T: int, n_envs: int, n_channels: int = 7, pin_memory: bool = True):
        self.T = int(T)
        self.n_envs = int(n_envs)
        pm = bool(pin_memory)
        # uint8 for obs (0/1)
        self.obs_u8 = torch.empty((T, n_envs, n_channels, 9, 9), dtype=torch.uint8, pin_memory=pm)
        self.next_obs_u8 = torch.empty((T, n_envs, n_channels, 9, 9), dtype=torch.uint8, pin_memory=pm)
        self.mask = torch.empty((T, n_envs, 81), dtype=torch.bool, pin_memory=pm)
        self.next_mask = torch.empty((T, n_envs, 81), dtype=torch.bool, pin_memory=pm)
        self.actions = torch.empty((T, n_envs), dtype=torch.int64, pin_memory=pm)
        self.logp_old = torch.empty((T, n_envs), dtype=torch.float32, pin_memory=pm)
        self.values = torch.empty((T, n_envs), dtype=torch.float32, pin_memory=pm)
        self.rewards = torch.empty((T, n_envs), dtype=torch.float32, pin_memory=pm)
        self.dones = torch.empty((T, n_envs), dtype=torch.float32, pin_memory=pm)

    def to_device(self, device: torch.device, non_blocking: bool = True):
        # Convert obs to float32 on device (0/1)
        obs = self.obs_u8.to(device=device, dtype=torch.float32, non_blocking=non_blocking)
        next_obs = self.next_obs_u8.to(device=device, dtype=torch.float32, non_blocking=non_blocking)
        return {
            'obs': obs,
            'next_obs': next_obs,
            'mask': self.mask.to(device=device, non_blocking=non_blocking),
            'next_mask': self.next_mask.to(device=device, non_blocking=non_blocking),
            'actions': self.actions.to(device=device, non_blocking=non_blocking),
            'logp_old': self.logp_old.to(device=device, non_blocking=non_blocking),
            'values': self.values.to(device=device, non_blocking=non_blocking),
            'rewards': self.rewards.to(device=device, non_blocking=non_blocking),
            'dones': self.dones.to(device=device, non_blocking=non_blocking),
        }
...
@torch.no_grad()
def act_batch(model: nn.Module, x: torch.Tensor, m: torch.Tensor, temperature: float) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Batched stochastic action selection on GPU.

    Args:
      x: (B,C,9,9) float32 on device
      m: (B,81) bool on device
    Returns:
      actions: (B,) int64
      logp:    (B,) float32
      values:  (B,) float32
    ...
    if not torch.isfinite(logits).all():
        raise RuntimeError('act_batch(): non-finite logits')
    logp_all = F.log_softmax(logits, dim=-1)
    probs = logp_all.exp()
    # multinomial expects probs >=0 and rows sum ~1
...
def act(model: nn.Module, x: torch.Tensor, m: torch.Tensor, temperature: float) -> Tuple[int, float, float]:
    # Single-state wrapper (kept for eval/opponent if needed).
    a, logp, v = act_batch(model, x.unsqueeze(0), m.unsqueeze(0), temperature)
...
def collect_rollouts(
    ...
    """
    GPU-optimized rollout collection.

    Key changes (vs baseline):
      - 1 opponent per update (shared across all envs) => enables batching opponent inference too
      - rollout stored in a preallocated tensor buffer (no Python Transition objects)
      - learning-policy action selection is fully batched on GPU
      - buffer stored in pinned CPU memory then transferred once per update

    IMPORTANT: we only store transitions when it is the learning policy's turn, and we absorb
    opponent moves until the next learning turn or terminal => induced single-agent MDP.
    """
    assert rollout_steps % n_envs == 0, 'For GPU path, set rollout_steps divisible by n_envs'
    T = rollout_steps // n_envs

    rng = np.random.default_rng()
    envs = [UTTTEnv(micro_win_reward=float(micro_win_reward)) for _ in range(n_envs)]
    obs_state = [env.reset(seed=int(rng.integers(0, 10_000_000))) for env in envs]

    # --- choose ONE opponent for the whole update ---
    opponent_is_random = (rng.random() < p_vs_random)
    if opponent_is_random or len(opponent_pool) == 0:
        opp_model = None
        opp_name = 'random'
    else:
    ...
    stats = {
        ...
        'opp_elo_mean': float(elo.get(opp_name)), 'opp_elo_count': float(n_envs),
        'opp_stronger_frac': 1.0 if float(elo.get(opp_name)) > float(elo.get('current')) else 0.0,
        'opp_name': opp_name,
        'opp_is_random': 1.0 if opponent_is_random else 0.0,
    }

    # Per-env: which side is learning policy this episode? (X=+1 starts, O=-1 second)
    learning_sign = [1 if rng.integers(0, 2) == 0 else -1 for _ in range(n_envs)]

    buf = RolloutBuffer(T=T, n_envs=n_envs, n_channels=7, pin_memory=True)

    def _reset_env(e: int):
        o = envs[e].reset(seed=int(rng.integers(0, 10_000_000)))
        obs_state[e] = o
        learning_sign[e] = 1 if rng.integers(0, 2) == 0 else -1

    def _tally_terminal(e: int):
        env = envs[e]
        outcome = env.winner if env.winner is not None else 0
        score = float(outcome * learning_sign[e])
        stats['episodes'] += 1.0
        stats['wins'] += 1.0 if score > 0 else 0.0
        stats['losses'] += 1.0 if score < 0 else 0.0
        stats['draws'] += 1.0 if score == 0 else 0.0
        # Elo update on the actual opponent used this update
        elo.update('current', opp_name, score)

    for t in range(T):
        # 1) advance opponent moves so that each env is at learning player's turn (or terminal)
        for e in range(n_envs):
            ...
            while (not env.done) and (env.current_player != learning_sign[e]):
                o = obs_state[e]
                if opponent_is_random:
                    a_op = act_random(o['action_mask'], rng)
                else:
                    # opponent inference is not batched here (rarely the bottleneck).
                    # You can batch this too if you want, but it complicates control flow a bit.
                    x_op, m_op = obs_to_torch(o, device)
                    a_op, _, _ = act(opp_model, x_op, m_op, temperature=1.0)
                o2, _, _, _ = env.step(a_op)
                obs_state[e] = o2

        # 2) build batch for learning-policy decisions
        obs_list = []
        mask_list = []
        for e in range(n_envs):
            env = envs[e]
            if env.done:
                # will be reset at next iteration
                # Fill dummy (won't be used)
                o = obs_state[e]
            else:
                o = obs_state[e]
            obs_list.append(torch.from_numpy(o['obs']).to(dtype=torch.float32))
            mask_list.append(torch.from_numpy(o['action_mask']).to(dtype=torch.bool))

        # Note: non_blocking transfers require pinned memory on CPU.
        # Stack first, then pin when using CUDA.
        obs_cpu = torch.stack(obs_list, dim=0)
        mask_cpu = torch.stack(mask_list, dim=0)
        if device.type == "cuda":
            obs_cpu = obs_cpu.pin_memory()
            mask_cpu = mask_cpu.pin_memory()
        obs_dev = obs_cpu.to(device=device, non_blocking=True)
        mask_dev = mask_cpu.to(device=device, non_blocking=True)

        # 3) batched act
        a_dev, logp_dev, v_dev = act_batch(model, obs_dev, mask_dev, temperature=temperature)
        a_cpu = a_dev.to('cpu')
        logp_cpu = logp_dev.to('cpu')
        v_cpu = v_dev.to('cpu')

        # 4) step envs with learning actions, then absorb opponent until next learning turn or terminal
        for e in range(n_envs):
            env = envs[e]
            if env.done:
                # store a dummy transition that will be ignored by done=1 (keeps shapes consistent)
                # We immediately reset so next iteration stays valid.
                _tally_terminal(e)
                _reset_env(e)
                env = envs[e]

            o = obs_state[e]
            act_e = int(a_cpu[e].item())
            logp_e = float(logp_cpu[e].item())
            v_e = float(v_cpu[e].item())

            o2, shaping_r, _, _ = env.step(act_e)
            obs_state[e] = o2

            # Absorb opponent moves
            while (not env.done) and (env.current_player != learning_sign[e]):
                o_mid = obs_state[e]
                if opponent_is_random:
                    ...
                    a_op, _, _ = act(opp_model, x_op, m_op, temperature=1.0)
                o3, _, _, _ = env.step(a_op)
                obs_state[e] = o3

            done_flag = bool(env.done)
            final_r = 0.0
            if done_flag:
                outcome = env.winner if env.winner is not None else 0
                final_r = float(outcome * learning_sign[e])
                _tally_terminal(e)
                _reset_env(e)
            ...
            # store into buffer (CPU pinned)
            buf.obs_u8[t, e].copy_(torch.from_numpy(o['obs']).to(torch.uint8))
            buf.mask[t, e].copy_(torch.from_numpy(o['action_mask']).to(torch.bool))
            buf.actions[t, e] = act_e
            buf.logp_old[t, e] = logp_e
            buf.values[t, e] = v_e
            buf.rewards[t, e] = total_r
            buf.dones[t, e] = 1.0 if done_flag else 0.0
            buf.next_obs_u8[t, e].copy_(torch.from_numpy(o_next['obs']).to(torch.uint8))
            buf.next_mask[t, e].copy_(torch.from_numpy(o_next['action_mask']).to(torch.bool))
...
def compute_gae(
    transitions: Union[List[Transition], RolloutBuffer],
    model: nn.Module,
    device: torch.device,
    gamma: float = 0.997,
    lam: float = 0.95,
    ...
    If `transitions` is a RolloutBuffer, uses vectorized (T,n_envs) computation on GPU.
    If it's a List[Transition], falls back to the old slower path (kept for compatibility).
    """
    if isinstance(transitions, RolloutBuffer):
        tb = transitions.to_device(device)
        obs = tb['obs']          # (T,E,C,9,9)
        masks = tb['mask']       # (T,E,81)
        actions = tb['actions']  # (T,E)
        logp_old = tb['logp_old']
        values = tb['values']
        rewards = tb['rewards']
        dones = tb['dones']      # float32
        next_obs = tb['next_obs']
        next_masks = tb['next_mask']

        T, E = rewards.shape

        with torch.no_grad():
            # bootstrap values for next states
            flat_next_obs = next_obs.reshape(T * E, *next_obs.shape[2:])
            flat_next_masks = next_masks.reshape(T * E, -1)
            next_logits, next_v = model(flat_next_obs)
            next_v = next_v.reshape(T, E)

        adv = torch.zeros((T, E), device=device, dtype=torch.float32)
        gae = torch.zeros((E,), device=device, dtype=torch.float32)
        for t in range(T - 1, -1, -1):
            nonterminal = 1.0 - dones[t]
            delta = rewards[t] + gamma * nonterminal * next_v[t] - values[t]
            gae = delta + gamma * lam * nonterminal * gae
            adv[t] = gae

        returns = adv + values

        # flatten to (N,...) for PPO
        obs_f = obs.reshape(T * E, *obs.shape[2:])
        masks_f = masks.reshape(T * E, -1)
        actions_f = actions.reshape(T * E)
        logp_f = logp_old.reshape(T * E)
        returns_f = returns.reshape(T * E)
        adv_f = adv.reshape(T * E)
        return obs_f, masks_f, actions_f, logp_f, returns_f.detach(), adv_f.detach()

    # ---- Compatibility slow path (original) ----
    assert isinstance(transitions, list)
    N = len(transitions)
    obs = torch.stack([t.obs for t in transitions], dim=0).to(device)
    masks = torch.stack([t.mask for t in transitions], dim=0).to(device)
    actions = torch.tensor([t.action for t in transitions], device=device)
    logp_old = torch.tensor([t.logp for t in transitions], device=device)
    values = torch.tensor([t.value for t in transitions], device=device)
...
def train(
    ...
    for upd in range(1, total_updates + 1):
        ...
        # Collect rollouts
        model.eval()
        buffer, roll_stats = collect_rollouts(
            model=model,
            opponent_pool=opponent_pool,
            elo=elo,
            device=device,
        ...
        with torch.no_grad():
            ...
            logp_all = F.log_softmax(logits, dim=-1)
            logp_old = logp_all.gather(1, actions[:, None]).squeeze(1)
