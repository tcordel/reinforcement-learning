...
public class DQN {
	...
	private void train() {
		...
		for (int i = 0; i < numEpisodes; i++) {
			...
			for (int ib = 0; ib < optimizationIteration; ib++) {
				optimizeSGD(samples.subList(ib * optimizationIteration, (ib + 1) *
	...
	private void initAdam() {
		...
		mb1 = new double[qModel.thetas[0].length];
		vb1 = new double[qModel.thetas[0].length];
		mb2 = new double[qModel.thetas[1].length];
		vb2 = new double[qModel.thetas[1].length];
	}
	...
	private void optimizeAdam(List<Dump> subList) {
		...
		updateBiasesAdamW(qModel.thetas[0], gradB1, mb1, vb1);
		updateBiasesAdamW(qModel.thetas[1], gradB2, mb2, vb2);
	}
