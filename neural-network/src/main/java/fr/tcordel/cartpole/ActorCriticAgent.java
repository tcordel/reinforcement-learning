...
public class ActorCriticAgent {
	...
	public ActorCriticAgent() {
		actor = new QNet(4, colsB, 2, 0.001);
		critic = new QNet(4, colsB, 2, 0.001);
		memory = new ReplayMemory(10000);
	}

	public static void main(String[] args) {
		new ActorCriticAgent().train(new CartPole(), 15000);
	...
	private Map<String, double[][]> computeCriticGradients(QNet model, List<double[][]> batch) {
		...
		double[][] hidden = relu(
				QNet.matrixAddForBias(matrixDot(states, model.weights1, batchSize, colsB, 4), model.bias1));
		double[][] qVals = QNet.matrixAddForBias(matrixDot(hidden, model.weights2, batchSize, 2, colsB), model.bias2);
		...
		double[][] gradW2 = matrixDot(transpose(hidden, batchSize, colsB), lossGrad, colsB, 2, batchSize);
		double[] gradB2 = sumRows(lossGrad);

		double[][] hiddenGrad = matrixDot(lossGrad, transpose(model.weights2, colsB, 2), batchSize, colsB, 2);
		for (int i = 0; i < batchSize; i++) {
		...
		double[][] gradW1 = matrixDot(transpose(states, batchSize, 4), hiddenGrad, 4, colsB, batchSize);
	...
	private Map<String, double[][]> computeActorGradients(QNet model, List<double[][]> batch) {
		...
		double[][] hidden = relu(
				QNet.matrixAddForBias(matrixDot(states, model.weights1, batchSize, colsB, 4), model.bias1));
		double[][] logits = QNet.matrixAddForBias(matrixDot(hidden, model.weights2, batchSize, 2, colsB), model.bias2);
		double[][] probs = softmax(logits);

		double[][] valuesHidden = relu(
				QNet.matrixAddForBias(matrixDot(states, critic.weights1, batchSize, colsB, 4), critic.bias1));
		double[][] values = QNet.matrixAddForBias(matrixDot(valuesHidden, critic.weights2, batchSize, 2, colsB),
				critic.bias2);
		...
		double[][] gradW2 = matrixDot(transpose(hidden, batchSize, colsB), dLogit, colsB, 2, batchSize);
		double[] gradB2 = sumRows(dLogit);

		double[][] hiddenGrad = matrixDot(dLogit, transpose(model.weights2, colsB, 2), batchSize, colsB, 2);
		for (int i = 0; i < batchSize; i++) {
			for (int j = 0; j < colsB; j++) {
				hiddenGrad[i][j] *= (hidden[i][j] > 0 ? 1 : 0);
			}
